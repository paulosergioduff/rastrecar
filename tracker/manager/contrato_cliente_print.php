...
$cons_cliente = mysqli_query($conn,"SELECT * FROM clientes WHERE id_cliente='$id_cliente'");
	...
	header('Location: /tracker2/index.php');
	}
	if(mysqli_num_rows($cons_cliente) > 0){
...
$assinatura = 	$resp_cliente['assinatura'];
	...
	$sql11 = mysqli_query($conn,"SELECT * FROM veiculos_clientes WHERE nr_contrato = '$nr_contrato' ORDER BY id_veiculo DESC");
	if(mysqli_num_rows($sql11) > 0){						
	while($rows44 = $sql11->fetch_array(MYSQLI_ASSOC)){
	...
	$id_veiculo = $rows44['id_veiculo'];
	}}


	$cons_os = mysqli_query($conn,"SELECT * FROM ordem_servico WHERE id_veiculo='$id_veiculo'");
	if(mysqli_num_rows($cons_os) > 0){
while ($resp_os = mysqli_fetch_assoc($cons_os)) {
$valor_os = 	$resp_os['valor'];
$valor_os = number_format($valor_os, 2, ",", ".");
	}}

	$sql1 = mysqli_query($conn,"SELECT * FROM clientes_contratos WHERE nr_contrato='$nr_contrato' ORDER BY nr_contrato ASC");
	if(mysqli_num_rows($sql1) > 0){						
	while($rows4 = $sql1->fetch_array(MYSQLI_ASSOC)){
	$pacote_cliente = $rows4['pacote'];
	$tipo_fatura = $rows4['tipo_fatura'];
	$valor_contrato = $rows4['valor'];
	$forma_pagamento = $rows4['forma_pagamento'];
	$dia_vencimento = $rows4['dia_vencimento'];
	$data_assinatura1 = $rows4['data_assinatura'];
	$data_assinatura = date('d/m/Y', strtotime("$data_assinatura1"));
	$data_vencimento1 = $rows4['data_vencimento'];
	$data_vencimento1 = date('d/m/Y', strtotime("$data_vencimento1"));
	$status = $rows4['status'];
	$data1 = strftime('%d de %B de %Y', strtotime($data_assinatura1));
	}}
...
$obs = 'Dia Venc. Boleto: '.$dia_vencimento.'';
}
		...
		<div class="ibox-content p-xl break" style="border-radius:5px; border:#000000 2px solid;">
			<div class="row">
				<div class="col-md-3">
					<div class="form-group">
						<img src="../Imagens/logo2.png" width="161" height="75" />
					</div>
				</div>
				...
				<div class="col-md-3 text-right" style="color:#000;">
					<div class="form-group">
						<span>Contrato: <?php echo $nr_contrato?></span>
					</div>
				</div>
			</div>
			...
			<div class="row">
				<div class="col-md-6" style="color:#000;">
					<div class="form-group">
						<p>Razão Social: <b>JC RASTREAMENTO</b></p>
						<p>CNPJ: <b>27.050.542/0001-25</b></p>
						<p>Endereço: <b>AV PRESIDENTE CASTELO BRANCO, 4251</b></p>
					</div>
				</div>
				<div class="col-md-6" style="color:#000;">
					<div class="form-group">
						<p>Telefone: <b>0800 042 0128</b></p>
						<p>Insc. Estadual: <b>06.638783-3</b></p>
						<p>Bairro/Cidade: <b>CENTRO, HORIZONTE/CE</b></p>
					</div>
				</div>
			</div>
			<hr style="border:#000 1px solid;">	
			...
			<?php
					...
					$result_usuario = "SELECT * FROM veiculos_clientes WHERE id_cliente = '$id_cliente' AND nr_contrato='$nr_contrato' ORDER BY id_veiculo DESC";
				...
				<div class="col-md-6" style="color:#000;">
					<div class="form-group">
						<p>Marca / Modelo: <b><?php echo $marca_veiculo?> / <?php echo $modelo_veiculo?></b></p>
						<p>Cor: <b><?php echo $cor_veiculo?></b></p>
						<p>Chassi: <b><?php echo $chassi?></b></p>
						<p>Categoria: <b><?php echo $tipo_veiculo?></b></p>
					</div>
				</div>
				<div class="col-md-6" style="color:#000;">
					<div class="form-group">
						<p>Ano: <b><?php echo $ano_veiculo?></b></p>
						<p>Renavam: <b><?php echo $renavan?></b></p>
						<p>Placa: <b><?php echo $placa?></b></p>
						<p>Combustível: <b><?php echo $combustivel?></b></p>
					</div>
				</div>
			</div>
			...
			<div class="row">
				<div class="col-md-6" style="color:#000;">
					<div class="form-group">
						<p>Periodicidade: <b><?php echo $tipo_fatura?></b></p>
						<p>Forma de Pagamento: <b><?php echo $forma_pagamento?></b></p>
						<p>Período de Vigência: <b><?php echo $data_assinatura?> até <?php echo $data_vencimento1?></b></p>
					</div>
				</div>
				<div class="col-md-6" style="color:#000;">
					<div class="form-group">
						<p>Valor Mensal: <b><?php echo $valor1?></b></p>
						<p>Espécie de Contrato: <b>Comodato de Equipamento</b></p>
						<p>Adesão: <b>R$ <?php echo $valor_os?></b></p>
