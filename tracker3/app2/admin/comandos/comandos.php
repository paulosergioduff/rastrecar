...
if($response_code == 200){
	...
	if($modelo_equip == 'CJ780' or $modelo_equip == 'ST901'){
		if($comando == 'BLOQUEIO'){
			$sql_veiculo = mysqli_query($conn, "UPDATE veiculos_clientes SET bloqueio='SIM' WHERE deviceid='$deviceid'");
		}
		if($comando == 'DESBLOQUEIO'){
			$sql_veiculo = mysqli_query($conn, "UPDATE veiculos_clientes SET bloqueio='NAO' WHERE deviceid='$deviceid'");
		}
	}
	
	header('Location: ../grid_device.php?deviceid='.$deviceid.'&id='.$id_push.'&res=200');
} 
...
if($response_code == 202){
	...
	if($modelo_equip == 'CJ780' or $modelo_equip == 'ST901'){
     ...
     header('Location: ../grid_device.php?deviceid='.$deviceid.'&id='.$id_push.'&res=202&cmd=enable');
} 

if($response_code == 400){
	header('Location: ../grid_device.php?deviceid='.$deviceid.'&id='.$id_push.'&res=400');
}
if($response_code == 401){
	header('Location: ../grid_device.php?deviceid='.$deviceid.'&id='.$id_push.'&res=401');;
}
