...
$stmt = $conn->prepare("SELECT COUNT(*) AS allcount FROM veiculos_clientes WHERE deviceid != '' AND (status='1' OR status='99' OR status='7') AND id_empresa='1361'");
$stmt->execute();
$records = $stmt->fetch();
$totalRecords = $records['allcount'];

## Total number of records with filtering
$stmt = $conn->prepare("SELECT COUNT(*) AS allcount FROM veiculos_clientes WHERE 1 ".$searchQuery." AND deviceid != '' AND (status='1' OR status='99' OR status='7') AND id_empresa='1361'");
$stmt->execute($searchArray);
$records = $stmt->fetch();
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$stmt = $conn->prepare("SELECT * FROM veiculos_clientes WHERE 1 ".$searchQuery." AND deviceid >= '1' AND (status='1' OR status='99' OR status='7') AND id_empresa='1361' ORDER BY ".$columnName." ".$columnSortOrder." LIMIT :limit,:offset");
