...
$date = date('Y-m-d');
$date_rel = date('Y-m-d 00:00:00');
$date_rel2 = date('Y-m-d H:i:s');
...
if(mysqli_num_rows($cons_cerca) > 0){
	...
	?>
                                        ...
                                        <div class="panel-content">
											...
											<div class="row">
												<div class="col-4 d-sm-flex align-items-center">
													<div class="p-1 mr-1">
													  <span class="badge" style="background-color:#000; color:#FFF; width:30px"><i class="fas fa-mobile fa-2x"></i></span>
													</div>	
													<div>
														<label class="fs-sm mb-0"><b>Dispositivo:</b><br><?php echo $modelo_equip?></label>
													</div>
												</div>
												<div class="col-4 d-sm-flex align-items-center">
													<div class="p-1 mr-1">
													  <span class="badge" style="background-color:#000; color:#FFF;width:30px"><i class="fas fa-qrcode fa-2x"></i></span>
													</div>	
													<div>
														<label class="fs-sm mb-0"><b>IMEI:</b><br><?php echo $imei?></label>
													</div>
												</div>
												<div class="col-4 d-sm-flex align-items-center">
													<div class="p-1 mr-1">
													  <span class="badge" style="background-color:#000; color:#FFF;width:30px"><i class="fas fa-terminal fa-2x"></i></span>
													</div>	
													<div>
														<label class="fs-sm mb-0"><b>ICCID:</b><br><?php echo $iccid?></label>
													</div>
												</div>
											</div><br>
											<div class="row">
												<div class="col-4 d-sm-flex align-items-center">
													<div class="p-1 mr-1">
													  <span class="badge" style="background-color:#000; color:#FFF; width:30px"><i class="fas fa-sim-card fa-2x"></i></span>
													</div>	
													<div>
														<label class="fs-sm mb-0"><b>Linha:</b><br><?php echo $chip?></label>
													</div>
												</div>
												<div class="col-4 d-sm-flex align-items-center">
													<div class="p-1 mr-1">
													  <span class="badge" style="background-color:#000; color:#FFF;width:30px"><i class="fas fa-phone-square fa-2x"></i></span>
													</div>	
													<div>
														<label class="fs-sm mb-0"><b>Operadora:</b><br><?php echo $operadora?></label>
													</div>
												</div>
												<div class="col-4 d-sm-flex align-items-center">
													<div class="p-1 mr-1">
													  <span class="badge" style="background-color:#000; color:#FFF;width:35px"><i class="fas fa-hotel fa-2x"></i></span>
													</div>	
													<div>
														<label class="fs-sm mb-0"><b>Fornecedor:</b><br><?php echo $fornecedor_chip?></label>
													</div>
												</div>
											</div>
											<hr style="border:#CCC 1px solid;">
											<div class="row">
												<div class="col-md-12">
													<button type="button" class="btn btn-primary btn-sm shadow-0" data-toggle="modal" data-target="#comandos_gprs"><i class="fas fa-link"></i> Comandos GPRS</button> 
													<button type="button" data-toggle="modal" data-target="#comandos_sms" class="btn btn-dark btn-sm shadow-0"><i class="far fa-comment-alt"></i> Comandos SMS</button> 
													<button type="button" class="btn btn-warning btn-sm shadow-0" data-toggle="modal" data-target="#alertas"><i class="far fa-bell"></i> Eventos</button>
													<button type="button" class="btn btn-info btn-sm shadow-0" data-toggle="modal" data-target="#logs"><i class="fas fa-th-list"></i> Logs</button>
													<a href="relatorio_percurso1.php?veiculo=<?php echo $deviceid?>&data_inicial=<?php echo $date_rel?>&data_final=<?php echo $date_rel2?>" target="_blank"><button type="button" class="btn btn-info btn-sm shadow-0"><i class="fas fa-external-link-alt"></i> Histórico</button></a>
												</div>
											</div>
                                        ...
                                        <div class="panel-content">
											...
											<div id="botoes">
												<nav class="d-none d-sm-block">
													...
													<label for="menu_open" class="menu-open-button " data-toggle="tooltip" data-placement="right" title="Opções">
													...
													<a href="http://maps.google.com/maps?q=<?php echo $latit?>,<?php echo $longit?>&ll=<?php echo $latit?>,<?php echo $longit?>&z=17" target="_blank" class="menu-item btn" style="background-color:#4169E1" data-toggle="tooltip" data-placement="right" title="Ver no Google Maps">
									...
									<div class="card mb-5" style="border:#ccc 1px solid;">
										<div class="card-body p-3">
											...
											<div class="row">
												<div class="col-md-12">
													<?php
													$cons_comandos = mysqli_query($conn,"SELECT * FROM comandos_enviados WHERE deviceid='$deviceid' ORDER BY id_log DESC ");
											...
											<div class="form-group">
												<label>Selecione o Comando:</label>
												 ...
												 <option value="">Selecione</option>
													<?php
													$cons_categorias_contas_pagar = mysqli_query($conn,"SELECT * FROM comandos_equip WHERE equipamento='$modelo_equip' ORDER BY nome_comando ASC");
		...
		<input type="hidden" value="http://jctracker.com.br/tracker2/admin/posicao.php?p=<?php echo $link_base?>" id="flink">
        <!-- END Page Wrapper -->
        <!-- BEGIN Quick Menu -->
	...
	<input type="hidden" id="saldo_sms1" />
...
<script>
	var intervalo5 = setTimeout(function(){
	
		var deviceid2 = document.getElementById("deviceid1").value;
		$.get("/tracker3/manager/include/consulta_bloqueio.php?deviceid="+deviceid2, function(data2){
			$('#ret_bloc1').val(data2);
			$('#ret_bloc2').val(data2);
		});
	...
	var deviceid4 = document.getElementById("deviceid1").value;
	setInterval(()=> {
			
			
			$.ajax({
				url: "/tracker3/manager/include/consulta_bloqueio.php?deviceid="+deviceid4,
				success: (data3)=>{
					var loaded = document.getElementById("ret_bloc2").value;
					$('#ret_bloc2').val(data3);
						if(data3 != loaded){
							$('#'+data3).modal('show');
...
var iconBase = '/tracker2/Imagens/icons/';
    ...
    var xml = data.responseXML;
      ...
      Array.prototype.forEach.call(markers, function(markerElem) {
            ...
            if(mapboxMarkers[i].name == name){
                  ...
                  if(String(ign) != String(mapboxMarkers[i].ign)
                          || String(point) != String(mapboxMarkers[i].point)
							...
							var popup = new mapboxgl.Popup({ closeOnClick: false }).setLngLat([longitude, latitude]).setHTML(name)
          ...
          if(encontrou == false){
                  ...
                  var popup = new mapboxgl.Popup({ closeOnClick: false }).setLngLat([-96, 37.8]).setHTML(name)
                  var marker = new mapboxgl.Marker(el).setLngLat([longitude, latitude]).setPopup(popup).addTo(map);
