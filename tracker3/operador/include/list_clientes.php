...
$stmt = $conn->prepare("SELECT COUNT(*) AS allcount FROM clientes WHERE id_cliente_pai='$id_cliente_pai'");
$stmt->execute();
$records = $stmt->fetch();
$totalRecords = $records['allcount'];

## Total number of records with filtering
$stmt = $conn->prepare("SELECT COUNT(*) AS allcount FROM clientes WHERE 1 ".$searchQuery." AND id_cliente_pai='$id_cliente_pai'");
$stmt->execute($searchArray);
$records = $stmt->fetch();
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$stmt = $conn->prepare("SELECT * FROM clientes WHERE 1 ".$searchQuery." AND id_cliente_pai='$id_cliente_pai' ORDER BY ".$columnName." ".$columnSortOrder." LIMIT :limit,:offset");
