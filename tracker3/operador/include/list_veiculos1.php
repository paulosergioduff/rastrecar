...
$searchQuery = " ";
if($searchValue != ''){
	$searchQuery = "  AND (status='1' OR status='99' OR status='7') AND (placa LIKE :placa or 
...
$stmt = $conn->prepare("SELECT COUNT(*) AS allcount FROM veiculos_clientes WHERE (status='1' OR status='99' OR status='7') AND id_cliente_pai='$id_cliente_pai'");
$stmt->execute();
$records = $stmt->fetch();
$totalRecords = $records['allcount'];

## Total number of records with filtering
$stmt = $conn->prepare("SELECT COUNT(*) AS allcount FROM veiculos_clientes WHERE 1 ".$searchQuery." AND (status='1' OR status='99' OR status='7') AND id_cliente_pai='$id_cliente_pai'");
$stmt->execute($searchArray);
$records = $stmt->fetch();
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$stmt = $conn->prepare("SELECT * FROM veiculos_clientes WHERE 1 ".$searchQuery." AND (status='1' OR status='99' OR status='7') AND id_cliente_pai='$id_cliente_pai' ORDER BY ".$columnName." ".$columnSortOrder." LIMIT :limit,:offset");
