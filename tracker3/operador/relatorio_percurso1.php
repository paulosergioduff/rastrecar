...
<html lang="en">
    ...
    <body class="mod-bg-1 nav-function-fixed" onLoad="AutoCenter();callMapa();">
						...
						<div class="row">
							...
							<div class="col-xl-4 text-right">
								...
								<button type="button" onclick="imprimir();" class="btn btn-primary btn-sm" style="font-size:14px">Imprimir</button>
								  <button type="button" onclick="abrir();" class="btn btn-primary btn-sm" style="font-size:14px">Ver no Mapa</button>
							</div>
						</div>
...
$cons_posicao = mysqli_query($conn,"SELECT * FROM tc_positions WHERE deviceid='$deviceid' AND ( id >= '$posicao_inicial' AND id <= '$posicao_final') ORDER BY servertime ASC");
	if(mysqli_num_rows($cons_posicao) > 0){
		...
		$driver_num = $resp_posicao['driver_num'];
...
$('#forml').on('submit', function(e){
  $('#carregar').modal('show');
		...
		function abrir(){
			...
			window.open("http://rastreiamaisbrasil.com.br/tracker3/operador/mapa_rel2.php?id_relatorio="+id_relatorio+"&deviceid="+device_id+"&data_inicial="+data_ini+"&data_final="+data_fin, "minhaJanela", "height=700,width=1000");
		}
		
		function imprimir(){
		window.open("http://rastreiamaisbrasil.com.br/tracker3/operador/imprimir_relatorio.php?c="+base64, "minhaJanela", "height=700,width=1000");
		}
	</script>
</body>
</html>
